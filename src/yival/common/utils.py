...
async def parallel_completions(message_batches, model, max_tokens):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for messages in message_batches:
            ...
            if available_request_capacity > 0 and available_token_capacity >= payload['max_tokens']:
                available_request_capacity -= 1
                available_token_capacity -= payload['max_tokens']
                task = asyncio.ensure_future(fetch(session, url, headers, payload, available_token_capacity))
                tasks.append(task)
        ...
        for i, response in enumerate(responses):
            if response == "rate_limit":
                await asyncio.sleep(SECONDS_TO_PAUSE_AFTER_RATE_LIMIT_ERROR)
                responses[i] = await fetch(session, url, headers, message_batches[i], available_token_capacity)
